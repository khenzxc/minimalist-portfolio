...
export default function UnderDevelopment({ title, subtitle, moduleName, theme = 'dark' }) {
  ...
  return (
    <div 
      className={`w-full py-24 px-4 sm:px-6 lg:px-8 border-b border-gray-850 relative overflow-hidden flex flex-col items-center justify-center min-h-[420px] transition-colors duration-300 ${
        isDark ? 'bg-[#0D1117] text-white' : 'bg-white text-gray-900'
      }`}
      style={{ fontFamily: "'Space Grotesk', sans-serif" }}
    >
      {/* Background Grid Pattern */}
      <div className={`absolute inset-0 opacity-[0.04] pointer-events-none ${
        isDark ? 'bg-[radial-gradient(#FFF_1.2px,transparent_1.2px)]' : 'bg-[radial-gradient(#000_1.5px,transparent_1.5px)]'
      ...
      <div className="relative z-10 text-center max-w-xl space-y-4">
        ...
        <p className={`text-sm font-mono max-w-md mx-auto leading-relaxed ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
        ...
        <div className="pt-6 max-w-xs mx-auto">
          <div className={`w-full border rounded p-3 text-left font-mono text-[10px] space-y-1 ${
            isDark ? 'bg-neutral-900/60 border-neutral-800 text-gray-500' : 'bg-gray-50 border-gray-200 text-gray-400'
          }`}>
            <div className={`flex justify-between ${isDark ? 'text-amber-400/70' : 'text-blue-500/70'}`}>
              <span>&gt; initializing_buffer_stream...</span>
              <span className="animate-pulse">LOADING</span>
            </div>
            <div className={`w-full h-1.5 rounded-full overflow-hidden mt-1 ${isDark ? 'bg-neutral-800' : 'bg-gray-200'}`}>
